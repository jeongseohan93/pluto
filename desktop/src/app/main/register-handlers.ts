 ...
 *
 * Nothing here decides anything. It exists so the two things that *are*
 * decisions — building an argv (`domains/pipeline/commands.ts`) and reading the
 * Function DB (`domains/graph-view/main/graph-store.ts`) — have exactly one
...
import { readGraphIndex, readGraphNode } from '../../domains/graph-view/main/graph-store'
